...
1. **Только русская локализация** — все строки в JSX. Для EN/других нужен `i18n-js` или `react-i18next`.
2. **Widget в настройках декоративный** — настоящий iOS/Android home-widget требует нативного кода + EAS Build с кастомными модулями.
3. **Web не поддерживает уведомления и SQLite** — `Platform.OS === 'web'` early-return. На телефоне через Expo Go всё ок.
